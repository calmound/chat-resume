...
export default function Sider({ onSend, loading }) {
  const handleFinish = (values: QueryType) => {
    const limitFetch = JSON.parse(localStorage.getItem('limitFetch') || '{}')
    // 在一天请求次数大于10次
    if (limitFetch?.count >= 10 && dayjs().diff(dayjs(limitFetch.day), 'days') < 1) {
      return message.warning('次数限制，明天再尝试')
    } else {
      localStorage.setItem('limitFetch', JSON.stringify({
        day: dayjs().valueOf(),
  ...
  return <div className={styles.sider}>
    ...
    <div className={styles.tips1}>有问题或者需要免费点评简历，微信: sanmu1598</div>
    <Image className={styles.img} src="/wechat.png" width={100} alt="" height={100} />
    <div className={styles.tips1}>key的使用存在一定费用，限制了次数。</div>
    <div className={styles.tips1}>若一直无法返回结果，则表明限流。收藏本站在低峰使用。</div>
    <div className={styles.tips2}>可以通过右上角设置自己的key，不限制使用次数。</div>
    <Form onFinish={handleFinish} layout="vertical" size="small">
      <Form.Item name="title" label="项目名称" rules={[{ required: true, message: '请输入标题' }]}>
      ...
      <Form.Item name="techStacks" label="技术栈" rules={[{ required: true, message: '请选择技术栈' }]}>
        <Select mode="tags"
          placeholder="选择你项目技术栈"
          maxTagCount={5}

          filterOption={(inputValue, option) => {
            return (option?.value as string).toLowerCase().includes(inputValue.toLowerCase())
          }}
        >
      ...
      <Button loading={loading} htmlType='submit' size='middle' className={styles.button}>生成</Button>
    </Form>
  </div >
}